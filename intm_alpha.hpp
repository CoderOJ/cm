...
template <class type_list, size_t id> struct __tl_get
{
  static_assert(id < type_list::size, "out of bound");
  using type = typename __tl_get<typename type_list::cdr, id - 1>::type;
};
template <class type_list> struct __tl_get<type_list, 0>
{
  using type = typename type_list::car;
};

template <class... T> struct type_list_t
{
};
template <class A, class... T> struct type_list_t<A, T...>
{
  using car = A;
...
template <int __mod, i64 __r> struct intm_r : public intm_tag
{
...
template <class mm> struct __get_mm_adjust
{
  using type = typename std::conditional<
      mm::car::range::r() < mm::car::mod, typename mm::car,
      typename __get_mm_adjust<typename mm::cdr>::type>::type;
};
template <> struct __get_mm_adjust<type_list_t<>>
{
  using type = void;
};

template <int __mod> struct intm : public intm_tag
{
  static constexpr int mod = __mod;
...
struct add_fitin
{
  template <class ra, class rb> static constexpr bool check()
  {
...
template <int __mod> struct mod_method
{
  static constexpr int mod = __mod;
};
template <int mod, class input_range> struct mm_nothing : public mod_method<mod>
{
...
template <class lhs_mm, class rhs_mm, class fitin_pred> struct __getmm_ii
{
...
template <class lhs_mm, class rhs_mm_list, class fitin_pred> struct __getmm_il
{
